...
- 欢迎界面：单人模式、AI 对战模式、限时挑战模式、**本地多人模式**、更换时装、设置、退出游戏。
- 设置界面：N 步自动增长开关，20x20 / 50x50 / 100x100 地图，多档分辨率（1280x720~2560x1440），全屏开关，音乐开关，音效开关。
- 键盘控制：W/A/S/D 控制方向，E 射箭，1 加速，2 减速，P 暂停，R 重新开始。
- **本地多人**：P1 使用 WASD+E，P2 使用方向键+/，90 秒限时对战，道具池复用 AI 对战模式。
- 开局等待：进入游戏后先加载地图，单人模式检测到 W/A/S/D 后开始；多人模式需双方同时按下方向键。
- 大地图适配：50x50/100x100 使用局部视口，以玩家蛇头为焦点。格子最小像素 18px。
- AI 对战道具：弓箭（飞行物逐格移动）、护盾、尖刺陷阱、减速时钟。
- AI 难度：低/中/高三档，高难度主动争夺道具和寻找射击机会。
- **随机事件（多样模式）**：每 30s 触发地图轰炸或刀光箭影，本地多人最后 30s 加速为每 5s 一次。
...
所有更新通过 Git 提交记录追踪，关键版本节点如下：

| 日期 | 提交 | 内容 |
|------|------|------|
| 2026-06-12 | `2e8229c` | UI全面美化：暗色主题+粒子系统+动画过渡 |
| 2026-06-12 | `0e740a2` | 新增随机事件系统：地图轰炸+刀光箭影，多样模式专属 |
| 2026-06-12 | `8118fc0` | 初始提交：单人/AI/限时/本地多人模式，三套皮肤，多分辨率+全屏，音频系统 |

后续每次功能更新请参考 `git log` 提交历史。
