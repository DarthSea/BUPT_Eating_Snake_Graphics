    ...
    bool paused, bool waitingForStart, int countdownMs)
{
    ...
    if (waitingForStart) {
        ...
        if (isMulti && countdownMs > 0) {
            int sec = (countdownMs + 999) / 1000;
            TCHAR cd[8];
            _stprintf_s(cd, 8, _T("%d"), sec);
            drawCenteredText(BOARD_LEFT, BOARD_TOP, BOARD_LEFT + boardSize, BOARD_TOP + boardSize,
                cd, (int)(72 * hudScale), COLOR_SCORE);
        } else if (isMulti) {
            drawTextAt(BOARD_LEFT + 8, BOARD_TOP + 8,
                _T("双方按下操控键开始"), 18, COLOR_SCORE);
        } else {
            const TCHAR *text;
...
void Render_drawControlSelectP2(RenderContext *render, int selected)
{
    ...
    setfillcolor(COLOR_BG);
    solidrectangle(0, 0, gWindowWidth, gWindowHeight);
    drawCenteredText(0, 48, gWindowWidth, 100, _T("玩家二 选择操控方式"), 38, COLOR_TEXT);
    drawCenteredText(0, 110, gWindowWidth, 140,
        _T("W/S/方向键 选择，Enter 确认，Esc 返回"), 18, COLOR_TEXT_DIM);
